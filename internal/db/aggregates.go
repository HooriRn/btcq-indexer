...
func (a *aggregateDescription) AddFirstExpression(name string, expression string) *aggregateDescription {
	return a.addExpression(name, expression, firstAggregateColumn)
}
...
func (a *aggregateDescription) AddMinExpression(name string, expression string) *aggregateDescription {
	return a.addExpression(name, expression, minAggregateColumn)
}

func (a *aggregateDescription) AddMaxExpression(name string, expression string) *aggregateDescription {
	return a.addExpression(name, expression, maxAggregateColumn)
}
...
func (agg *aggregateDescription) baseQueryBuilder(b io.Writer, aggregateTimestamp string, whereConditions []string, groupColumns []string) {
	...
	for _, c := range agg.columns {
		...
		if c.columnType == lastAggregateColumn {
			expression = "last(" + expression + ", " + agg.table + ".block_timestamp)"
		}
		if c.columnType == firstAggregateColumn {
			expression = "first(" + expression + ", " + agg.table + ".block_timestamp)"
		}
		if c.columnType == minAggregateColumn {
			expression = "min(" + expression + ")"
		}
		if c.columnType == maxAggregateColumn {
