...
func getDepthsHistory(ctx context.Context, buckets db.Buckets, pools []string,
	...
	if buckets.OneInterval() {
		// We only interested in the state between the two timestamps
		poolDepths, err = DepthsPeriod(ctx, pools, buckets.Timestamps[0].ToNano(), buckets.Timestamps[1].ToNano())
		if err != nil {
			return nil, err
		}
...
func DepthsBefore(ctx context.Context, pools []string, time db.Nano) (
	...
	firstValueQuery := `
		SELECT
			...
			last(close_price, aggregate_timestamp) AS low_price,
			last(close_price, aggregate_timestamp) AS close_price
		FROM ` + subQuery + ` AS u
		GROUP BY pool
	`
...
func DepthsPeriod(ctx context.Context, pools []string, timeLow db.Nano, timeHigh db.Nano) (
	ret timeseries.DepthMap, err error) {
	...
	subQuery, qargs := poolDepthsAggregate.UnionQuery(timeLow, timeHigh, whereConditions, qargs)
	...
	// For pools that have no results in the period, get the latest values before the period
	if pools != nil {
		var missingPools []string
		for _, pool := range pools {
			_, present := ret[pool]
			if !present {
				missingPools = append(missingPools, pool)
			}
		}
		if len(missingPools) > 0 {
			beforeDepths, err := DepthsBefore(ctx, missingPools, timeLow)
			if err != nil {
				return nil, err
			}
			for pool, depths := range beforeDepths {
				ret[pool] = depths
			}
		}
