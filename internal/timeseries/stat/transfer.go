...
func PoolAddsLookup(ctx context.Context, pool string, w db.Window) (*PoolAdds, error) {
	const q = `SELECT COALESCE(SUM(asset_e8), 0), COALESCE(SUM(qbtc_e8), 0)
FROM add_events
WHERE pool = $1 AND block_timestamp >= $2 AND block_timestamp < $3`

	var r PoolAdds
	err := db.TheDB.QueryRow(q, pool, w.From.ToNano(), w.Until.ToNano()).Scan(&r.AssetE8Total, &r.QbtcE8Total)
	if err != nil {
...
func PoolErratasLookup(ctx context.Context, pool string, w db.Window) (*PoolErratas, error) {
	const q = `SELECT COALESCE(SUM(asset_e8), 0), COALESCE(SUM(qbtc_e8), 0) FROM errata_events
